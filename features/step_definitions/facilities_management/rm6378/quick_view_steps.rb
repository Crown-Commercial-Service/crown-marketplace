...
Then('there is a notification with the message {string}') do |notification_message|
  expect(quick_view_page.notification_banner.title).to have_text('Important')
  expect(quick_view_page.notification_banner.message).to have_text(notification_message)
end
