Then('the procurement name is shown to be {string}') do |contract_name|
  expect(page.find_by_id('procurement-subtitle')).to have_text(contract_name)
end

When('the contract number is visible') do
  contract_number = @procurement.contract_number

  expect(page.find('#main-content > div:nth-child(1) > div > span')).to have_text("#{@procurement.contract_name} - #{contract_number}")
  expect(page.find('.ccs-panel__body')).to have_text(contract_number)
end
