...
RSpec.describe ActiveStorage::Blobs::RedirectController do
  ...
  describe '#show' do
    context 'when trying to download an RM3830 management report' do
      ...
      let(:blob_object_id_key) { :rm3830_management_report_id }
      let(:blob_object_id) { management_report.id }
    ...
    context 'when trying to download an RM6232 management report' do
      let(:signed_id) { management_report.management_report_csv.blob.signed_id }
      let(:filename) { management_report.management_report_csv.blob.filename }
      let(:blob_object_id_key) { :rm6232_management_report_id }
      let(:blob_object_id) { management_report.id }
    ...
    context 'when trying to download an admin upload in RM3830' do
      ...
      let(:blob_object_id_key) { :fm_rm3830_upload_id }
    ...
    context 'when trying to download an admin upload in RM6232' do
      ...
      let(:blob_object_id_key) { :fm_rm6232_upload_id }
      let(:blob_object_id) { admin_upload.id }
    ...
    context 'when trying to download an admin upload in RM6378' do
      let(:signed_id) { admin_upload.supplier_details_file.blob.signed_id }
      let(:filename) { admin_upload.supplier_details_file.blob.filename }
      let(:blob_object_id_key) { :fm_rm6378_upload_id }
      let(:blob_object_id) { admin_upload.id }
      let(:admin_upload) { create(:facilities_management_rm6378_admin_upload_with_document) }
    ...
    context 'when trying to download a security policy document' do
      ...
      let(:blob_object_id_key) { :procurement_id }
      let(:blob_object_id) { procurement.id }
    ...
    context 'when trying to download the contract documents' do
      ...
      let(:blob_object_id_key) { :contract_id }
      let(:blob_object_id) { contract.id }
    ...
    context 'when trying to download a report' do
      let(:signed_id) { report.report_csv.blob.signed_id }
      let(:filename) { report.report_csv.blob.filename }
      let(:blob_object_id_key) { :report_id }
      let(:blob_object_id) { report.id }
      ...
      context 'when signed in as an fm admin' do
        login_fm_admin
