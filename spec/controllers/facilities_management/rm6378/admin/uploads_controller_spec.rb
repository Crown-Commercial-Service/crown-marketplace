...
RSpec.describe FacilitiesManagement::RM6378::Admin::UploadsController do
  ...
  describe 'POST create' do
    ...
    let(:upload) { create(:facilities_management_rm6378_admin_upload) }
    ...
    before do
      ...
      allow(FacilitiesManagement::RM6378::Admin::Upload).to receive(:new).with(anything).and_return(upload)
      allow(FacilitiesManagement::RM6378::Admin::FileUploadWorker).to receive(:perform_async).with(upload.id).and_return(true)
      post :create, params: { facilities_management_rm6378_admin_upload: { supplier_details_file: fake_file, supplier_rate_cards_file: fake_file, supplier_service_offerings_file: fake_file } }
    end
    ...
    context 'when the upload is valid' do
      ...
      it 'redirects to the show page' do
        expect(response).to redirect_to facilities_management_rm6378_admin_upload_path(upload)
      end
  ...
  describe 'GET show' do
    let(:upload) { create(:facilities_management_rm6378_admin_upload, aasm_state:) }
    ...
    context 'when the upload has failed' do
      ...
      it 'renders the show template and the failed partial' do
        expect(response).to render_template(:show)
        expect(response).to render_template(partial: 'facilities_management/rm6378/admin/uploads/_failed')
      end
    end
  end

  describe 'GET progress' do
    let(:upload) { create(:facilities_management_rm6378_admin_upload, aasm_state: 'publishing_data') }
