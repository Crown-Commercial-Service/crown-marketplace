...
RSpec.describe FacilitiesManagement::RM6232::ProcurementBuildingsController, type: :controller do
  ...
  context 'without buyer details' do
    ...
    it 'will redirect to buyer details' do
      get :edit, params: { id: procurement_building.id, section: 'buildings-and-services' }
  ...
  describe 'PUT update' do
    ...
    context 'when updating the services for the building' do
      let(:section_name) { 'buildings-and-services' }
      let(:model_name) { :facilities_management_rm6232_procurement_building }

      context 'and the data is valid' do
        let(:update_params) { { service_codes: %w[F.1 F.2] } }

        it 'redirects to the details show page' do
          expect(response).to redirect_to facilities_management_rm6232_procurement_procurement_detail_path(procurement, 'buildings-and-services')
        end
