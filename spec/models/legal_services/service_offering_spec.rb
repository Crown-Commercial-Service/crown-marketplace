...
RSpec.describe LegalServices::ServiceOffering, type: :model do
  subject(:service_offering) { build(:legal_services_service_offering) }
  ...
  it 'is not valid if offering exists for same lot_number, service_code & supplier' do
    ...
    new_offering = build(
      :legal_services_service_offering,
  ...
  it 'looks up service based on its code' do
    offering = build(:legal_services_service_offering, service_code: 'WPSLS.1.8')
    expect(offering.service).to be_instance_of(LegalServices::Service)
    expect(offering.service.code).to eq('WPSLS.1.8')
  end
end
