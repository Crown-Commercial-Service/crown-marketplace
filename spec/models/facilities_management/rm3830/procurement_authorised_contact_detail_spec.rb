...
RSpec.describe FacilitiesManagement::RM3830::ProcurementAuthorisedContactDetail, type: :model do
  ...
  describe '#telephone_number' do
    context 'when telephone_number is only one space character' do
      it 'expected to be invalid' do
        ...
        expect(procurement_authorised_contact_detail.valid?(:new_authorised_representative)).to eq false
      end
    end
    ...
    context 'when telephone_number is valid' do
      it 'expected to be valid' do
        expect(procurement_authorised_contact_detail.valid?(:new_authorised_representative)).to eq true
      end
    end
