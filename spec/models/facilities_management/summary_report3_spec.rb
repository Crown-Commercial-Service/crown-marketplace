...
RSpec.describe FacilitiesManagement::SummaryReport, type: :model do
  ...
  let(:procurement) { create(:facilities_management_procurement_with_extension_periods, user: user, initial_call_off_period: 7, procurement_buildings: [procurement_building, procurement_building_london]) }
  ...
  context 'when creating a direct-award report' do
    let(:spreadsheet_builder) { FacilitiesManagement::DeliverableMatrixSpreadsheetCreator.new contract.id }
    let(:spreadsheet) { spreadsheet_builder.build }

    it 'no errors' do
      IO.write('/tmp/deliverable_matrix_3.xlsx', spreadsheet.to_stream.read)
    end

    context 'with contract length of 1 year' do
      let(:da_spreadsheet) { FacilitiesManagement::DirectAwardSpreadsheet.new contract.id }

      it 'no errors' do
        IO.write('/tmp/direct_award_prices_3_1year.xlsx', da_spreadsheet.to_xlsx)
        IO.write('/tmp/deliverable_matrix_3_1year.xlsx', spreadsheet.to_stream.read)
      end

      # rubocop:disable RSpec/ExampleLength
      it 'verify a report for each building' do
        report_results = {}
        ...
        buildings_ids = procurement.active_procurement_buildings.map(&:building_id).flatten.uniq
    ...
    it 'only allowed volume worksheet services are displayed' do
      IO.write('/tmp/deliverable_matrix_3_1year.xlsx', spreadsheet.to_stream.read)
  ...
  describe FacilitiesManagement::DirectAwardSpreadsheet do
    subject(:workbook) do
      ...
      path = '/tmp/direct_award_prices_3.xlsx'
      IO.write(path, spreadsheet.to_xlsx)
      Roo::Excelx.new(path)
    end

    it 'create a direct-award report check service price cell position' do
      expect(workbook.sheet('Contract Price Matrix').row(24)[2]).to eq 21130.5
      expect(workbook.sheet('Contract Price Matrix').row(24)[3]).to eq 21130.5
    end
  end
end
