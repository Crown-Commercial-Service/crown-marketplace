...
RSpec.describe FacilitiesManagement::Procurement, type: :model do
  ...
  describe 'contract data should not be invalid because call of period is not > 0' do
    ...
    context 'when initial_call_off_period is < 0' do
      it 'will be invalid' do
        ...
        expect(procurement.valid?).to eq false
      end
    end
  ...
  describe 'when a call off period is specified, other validation rules will come into effect, ' do
    ...
    describe 'there are a set of rules that all depend on the gateway values being valid' do
      context 'when the call-off end date is empty' do
        it 'will be invalid' do
          procurement.initial_call_off_period = 1
          procurement.tupe = true
          procurement.mobilisation_period = 4
          ...
          procurement.save
          log_error
          expect(procurement.errors.details[:initial_call_off_end_date][0][:error]).to eq :not_a_date
          expect(procurement.valid?).to eq false
        end
      end

      context 'when the call-off end date is less than the start date' do
        it 'will be invalid' do
          procurement.initial_call_off_period = 1
          procurement.tupe = true
          procurement.mobilisation_period = 4
          ...
          procurement.save
          log_error
          expect(procurement.errors.details[:initial_call_off_end_date][0][:error]).to eq :after_or_equal_to
          expect(procurement.valid?).to eq false
        end
      end

      context 'when the call-off end date is more than/= to the start date' do
        it 'will be valid' do
          procurement.initial_call_off_period = 1
          procurement.tupe = true
          procurement.mobilisation_period = 4
