  ...
  describe '#valid_pension?' do
    context 'when the name and percentage are valid' do
      it 'is expected to be valid' do
        expect(pension_fund.valid_pension?).to eq true
      end
    end
    ...
    describe '#valid_name?' do
      ...
      context 'when the name is more than 150 characters' do
        it 'is expected to not be valid' do
          ...
          expect(pension_fund.send(:valid_name?)).to eq false
      ...
      context 'when the name is between 1 and 150 characters inclusive' do
        it 'is expected to be valid' do
          pension_fund.name = 'a'
          expect(pension_fund.send(:valid_name?)).to eq true
    ...
    describe '#valid_percentage?' do
      ...
      context 'when the percentage is less than 1' do
        it 'is expected to not be valid' do
          ...
          expect(pension_fund.send(:valid_percentage?)).to eq false
      ...
      context 'when the percentage is not a number' do
        let(:second_pension_fund) { build(:facilities_management_procurement_pension_fund, procurement: create(:facilities_management_procurement)) }

        it 'is expected to not be valid' do
          allow(second_pension_fund).to receive(:percentage) { (0...5).map { ('a'..'z').to_a[rand(26)] }.join }
          expect(second_pension_fund.send(:valid_percentage?)).to eq false
        end
      end
      ...
      context 'when the percentage is not a whole number' do
        ...
        it 'is expected to not be valid' do
          allow(second_pension_fund).to receive(:percentage) { (100 * rand(2..100) / 101.0) }
          expect(second_pension_fund.send(:valid_percentage?)).to eq false
        end
      end
      ...
      context 'when the percentage is between 1 and 100 percent inclusive' do
        it 'is expected to be false' do
          ...
          expect(pension_fund.send(:valid_percentage?)).to eq true
