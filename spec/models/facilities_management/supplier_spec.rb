  ...
  RSpec.describe Supplier, type: :model do
    ...
    describe '.delete_all_with_dependents' do
      ...
      it 'deletes all regional availabilities' do
        ...
        expect(RegionalAvailability.count).to eq(0)
      end

      it 'deletes all service offerings' do
        described_class.delete_all_with_dependents

        expect(ServiceOffering.count).to eq(0)
      end
    ...
    describe '#services_by_work_package_in_lot' do
      ...
      let(:service_a1) { Service.find_by(code: 'A.1') }
      let(:service_a2) { Service.find_by(code: 'A.2') }
      let(:service_b1) { Service.find_by(code: 'B.1') }

      let(:work_package_a) { WorkPackage.find_by(code: 'A') }
      let(:work_package_b) { WorkPackage.find_by(code: 'B') }
      let(:work_package_c) { WorkPackage.find_by(code: 'C') }
