...
RSpec.describe FacilitiesManagement::RM6378::Admin::Upload do
  ...
  describe 'validations' do
    context 'when considering if the files are attached' do
      ...
      context 'and one file is attached' do
        before do
        ...
        it 'is not valid and has the correct error messages' do
          expect(upload.valid?(:upload)).to be false
          expect(upload.errors.messages.values.flatten).to match ["Select the 'Supplier details' file", "Select the 'Supplier regions' file"]
        end
      end
    end
    ...
    context 'when considering the file extension' do
      ...
      context 'and two files have the correct file extension' do
        before do
        ...
        it 'is not valid and has the correct error messages' do
          expect(upload.valid?(:upload)).to be false
          expect(upload.errors.messages.values.flatten).to match ["The 'Supplier services' file must be an XLSX"]
        end
      end
    end
    ...
    context 'when considering the file content type' do
      ...
      context 'and one file has the correct content type' do
        before do
          upload.supplier_details_file = valid_file_path
          upload.supplier_services_file = invalid_file_path
          upload.supplier_regions_file = invalid_file_path
  ...
  describe 'aasm_state' do
    ...
    context 'when start_upload is called' do
      ...
      it 'starts the worker' do
        expect(FacilitiesManagement::RM6378::Admin::FileUploadWorker).to have_received(:perform_async).with(upload.id)
      end
    end
