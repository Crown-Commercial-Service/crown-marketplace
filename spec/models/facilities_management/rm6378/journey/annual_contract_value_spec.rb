...
RSpec.describe FacilitiesManagement::RM6378::Journey::AnnualContractValue do
  ...
  describe 'validations' do
    context 'when no Average estimated contract value is present' do
      let(:estimated_annual_contract_value) { nil }

      it 'is not valid and has the correct error message' do
        expect(annual_contract_value.valid?).to be false
        expect(annual_contract_value.errors[:annual_contract_value].first).to eq 'The Average estimated contract value must be a whole number greater than 0'
      end
    end

    context 'when the Average estimated contract value is not a number' do
    ...
    context 'when the Average estimated contract value is not an integer' do
    ...
    context 'when the Average estimated contract value is less than 1' do
    ...
    context 'when the Average estimated contract value is more than 999,999,999,999' do
      let(:estimated_annual_contract_value) { 1_000_000_000_000 }

      it 'is not valid and has the correct error message' do
        expect(annual_contract_value.valid?).to be false
        expect(annual_contract_value.errors[:annual_contract_value].first).to eq 'The Average estimated contract value must be less than 1,000,000,000,000 (1 trillion)'
      end
    end

    context 'when Average estimated contract value is present' do
      it 'is valid' do
