...
RSpec.describe FacilitiesManagement::RM6232::WorkPackage, type: :model do
  describe 'work packages' do
    it 'there are 18 work packages' do
      expect(described_class.count).to eq 18
    end

    it 'there are the correct work package codes' do
      expect(described_class.pluck(:code)).to match ('A'..'R').to_a
    end
  end

  describe '.selectable' do
    it 'there are 14 selectable work packages' do
      expect(described_class.selectable.count).to eq 14
    end

    it 'there are the correct work package codes' do
      expect(described_class.selectable.pluck(:code)).to match ('E'..'R').to_a
    end
  end
