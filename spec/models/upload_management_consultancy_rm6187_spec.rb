    ...
    context 'when supplier does not exist' do
      ...
      context 'and supplier is offering lot 1 and 2 services' do
        ...
        it 'assigns attributes to the framework lot services' do
          ...
          expect(supplier_framework.lots.map { |lot| lot.services.pluck(:service_id) }).to eq([['RM6187.1.1', 'RM6187.1.2'], ['RM6187.2.1']])
        end
      end
      ...
      context 'and supplier has rates' do
        ...
        it 'assigns attributes to the jurisdictions' do
          ...
          expect(supplier_framework.lots.map { |lot| lot.jurisdictions.pluck(:jurisdiction_id) }).to eq([['RM6187.GB'], ['RM6187.GB']])
        end

        it 'assigns attributes to the rates' do
          result

          expect(supplier_framework.lots.map { |lot| lot.rates.pluck(:position_id, :rate) }).to eq([[['RM6187.1.1', 10000]], [['RM6187.3.1', 20000]]])
        end
      end
    end
