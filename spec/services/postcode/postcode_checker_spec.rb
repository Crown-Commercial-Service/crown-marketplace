...
RSpec.describe Postcode::PostcodeChecker do
  describe 'POST postcode' do
    it 'Postcode not in London' do
      expect(described_class.in_london?('G32 0RP')[:result]).to be false
    end

    it 'Postcode in London' do
      expect(described_class.in_london?('SW1P 2BA')[:result]).to be true
    end

    it 'Bad postcode not in London' do
      expect(described_class.in_london?('X11 1XX')['error']).to eq('Postcode not found')
    end
