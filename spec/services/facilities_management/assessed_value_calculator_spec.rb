...
RSpec.describe FacilitiesManagement::AssessedValueCalculator do
  ...
  let(:procurement_lot1a) { create(:facilities_management_procurement_with_contact_details_with_buildings, user: user) }
  let(:procurement_lot1c) { create(:facilities_management_procurement_with_contact_details_with_buildings, user: user, lot_number: '1c', lot_number_selected_by_customer: true) }
