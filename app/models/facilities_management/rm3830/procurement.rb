module FacilitiesManagement
  module RM3830
    class Procurement < FacilitiesManagement::Procurement
      include AASM
      include ProcurementValidator
      include ServiceQuestionsConcern

      default_scope { where(framework: 'RM3830') }
      before_create { self.framework = 'RM3830' }

      scope :searches, -> { where(aasm_state: SEARCH).order(updated_at: :asc).sort_by { |search| SEARCH_ORDER.index(search.aasm_state) } }
      scope :in_draft, -> { da_draft.order(updated_at: :asc) }
      scope :sent_offers, -> { direct_award&.map(&:sent_offers)&.flatten&.sort_by { |contract| [FacilitiesManagement::ProcurementSupplier::SENT_OFFER_ORDER.index(contract.aasm_state), contract.offer_sent_date] } }
      scope :live_contracts, -> { direct_award.map(&:live_contracts)&.flatten&.sort_by(&:contract_signed_date) }
      scope :closed_contracts, -> { where(aasm_state: ['direct_award', 'closed']).map(&:closed_contracts)&.flatten&.sort_by { |sent_offer| [sent_offer.contract_closed_date ? 1 : 0, sent_offer.contract_closed_date] }&.reverse }
      scope :further_competition_contracts, -> { further_competition.order(updated_at: :asc) }
