module FacilitiesManagement
  class ProcurementContactDetail < ApplicationRecord
    ...
    validates :organisation_address_line_1, presence: true, on: %i[new_invoicing_contact_details_address new_authorised_representative_address new_notices_contact_details_address]
    validates :organisation_address_town, presence: true, on: %i[new_invoicing_contact_details_address new_authorised_representative_address new_notices_contact_details_address]

    validates :organisation_address_postcode, presence: true, format: { with: /\A([a-zA-Z (0-9)]*)\z/ }, on: %i[new_invoicing_contact_details new_authorised_representative new_notices_contact_details new_invoicing_contact_details_address new_authorised_representative_address new_notices_contact_details_address]
    validate  :address_entered_when_postcode_provided, on: %i[new_invoicing_contact_details new_authorised_representative new_notices_contact_details]
    validate :valid_postcode?, on: %i[new_invoicing_contact_details new_authorised_representative new_notices_contact_details new_invoicing_contact_details_address new_authorised_representative_address new_notices_contact_details_address], if: -> { organisation_address_postcode.present? }
