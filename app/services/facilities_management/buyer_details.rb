    ...
    class BuyerDetails
      ...
      def buyer_details(email)
        ...
        query = "select
                  ...
                  email
                from
                  public.facilities_management_buyer
                where active = true and email = '#{email}' order by created_at DESC limit 1; "
        results = ActiveRecord::Base.connection_pool.with_connection { |con| con.exec_query(query) }
        results
      rescue StandardError => e
