module FacilitiesManagement::Beta::Procurements::DocumentsProcurementHelper
  ...
  def download_all
    file_stream = build_download_zip_file(params[:procurement_id])
  ...
  def self.build_download_zip_file(procurement_id)
    ...
    direct_award_spreadsheet = FacilitiesManagement::DirectAwardSpreadsheet.new procurement_id
    deliverable_matrix_spreadsheet = FacilitiesManagement::DeliverableMatrixSpreadsheetCreator.new procurement_id
    ...
    file_stream = Zip::OutputStream.write_buffer do |zip|
      ...
      zip.put_next_entry 'Attachment 4 - Order Form and Call-off Schedule (DA) (1).docx'
      zip.print generate_doc(@procurement.first_unsent_contract.id)
    end
  ...
  def self.generate_doc(contract_id)
    ...
    view.render(file: 'facilities_management/beta/procurements/contracts/documents/call_off_schedule.docx.caracal')
  end

  def self.generate_final_zip(procurement_id)
    @procurement = FacilitiesManagement::Procurement.find(procurement_id)
    file_stream = self.build_download_zip_file(procurement_id)
