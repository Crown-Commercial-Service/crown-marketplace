...
module LayoutHelper
  ...
  def govuk_continuation_buttons(page_description, form_builder, secondary_button = true, return_link = true, primary_button = true, red_secondary_button = false, primary_btn_as_link = false, secondary_btn_as_link = false)
    ...
    buttons << form_builder.submit(page_description.navigation_details.primary_text, class: 'govuk-button govuk-!-margin-right-4', data: { disable_with: false }, name: [page_description.navigation_details.primary_name, 'commit'].find(&:present?), aria: { labelledby: page_description.navigation_details.primary_text }) if primary_button && !primary_btn_as_link
    ...
    buttons << link_to(page_description.navigation_details.secondary_text, page_description.navigation_details.secondary_url, class: "govuk-button #{red_secondary_button ? 'govuk-button--warning' : 'govuk-button--secondary'}", role: 'button', aria: { labelledby: page_description.navigation_details.secondary_text }) if secondary_button && secondary_btn_as_link
  ...
  def govuk_button(builder, text, options = { submit: true, class: '' })
    ...
    css_classes += text == ['aria-labelledby']
