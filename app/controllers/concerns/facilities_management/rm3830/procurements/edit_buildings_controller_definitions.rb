    ...
    module Procurements::EditBuildingsControllerDefinitions
      ...
      def page_definitions
        @page_definitions ||= {
          default: {
            ...
            continuation_url: new_facilities_management_rm3830_procurement_edit_building_path(procurement_id: @procurement),
            ...
            secondary_url: edit_facilities_management_rm3830_procurement_path(@procurement.id, step: 'buildings'),
            back_text: I18n.t('facilities_management.buildings.page_definitions.return_to_buildings'),
            back_url: edit_facilities_management_rm3830_procurement_path(@procurement.id, step: 'buildings')
          },
          new: {
      ...
      def edit_details
        ...
        details = {
          ...
          secondary_name: 'save_and_return',
          secondary_text: I18n.t('facilities_management.buildings.page_definitions.save_and_return_to_detailed_summary'),
          back_url: facilities_management_rm3830_procurement_edit_building_path(@page_data[:model_object].id, procurement_id: @procurement.id),
        ...
        if %w[gia type security].include? params[:step]
          ...
          details[:back_url] = edit_facilities_management_rm3830_procurement_edit_building_path(@page_data[:model_object].id, procurement_id: @procurement.id, step: previous_step(params[:step].to_sym))
        end
