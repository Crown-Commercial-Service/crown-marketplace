    ...
    module Procurements
      class ContractsController < FacilitiesManagement::FrameworkController
        ...
        def redirect_if_contract_cannot_be_updated
          redirect_to facilities_management_rm3830_procurement_contract_path if cannot_edit_or_update_contract?
        end
        ...
        def redirect_if_unrecognised_name
          redirect_to facilities_management_rm3830_procurement_contract_path(procurement_id: @procurement.id, id: @contract.id) unless RECOGNISED_NAMES.include? params[:name]
        end
