module FacilitiesManagement
  module Procurements
    class EditBuildingsController < FacilitiesManagement::FrameworkController
      ...
      include FacilitiesManagement::PageDetail::EditBuildings
      ...
      def add_address_form_details
        @add_address_form_details ||= if id_present?
                                        { url: facilities_management_procurement_edit_building_path(@page_data[:model_object].id, procurement_id: params[:procurement_id]), method: :patch }
                                      else
      ...
      def redirect_if_unrecognised_step
        redirect_to facilities_management_procurement_edit_building_path(params[:procurement_id], @page_data[:model_object]) unless RECOGNISED_STEPS.include? params[:step]
      end
