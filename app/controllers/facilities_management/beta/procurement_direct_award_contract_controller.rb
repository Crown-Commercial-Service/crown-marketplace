    ...
    class ProcurementDirectAwardContractController < FacilitiesManagement::Beta::FrameworkController
      ...
      def show
        @page_data[:procurement_data] = { contract_name: 'School facilities London', supplier: 'Cleaning London LTD', date_offer_expires: DateTime.new(2019, 7, 7, 8, 2, 0).in_time_zone('London'), contract_number: 'RM330-DA2234-2019', contract_value: 752026, framework: 'RM3830', sub_lot: 'sub-lot 1a',
                                          ...
                                          contract_start_date: Date.new(2019, 7, 17), contract_end_date: Date.new(2026, 7, 16), contract_not_signed_date: DateTime.new(2019, 6, 23, 14, 20, 0).in_time_zone('London'), date_contract_declined: DateTime.new(2019, 11, 23, 12, 30, 0).in_time_zone('London'), contract_declined_date: DateTime.new(2019, 06, 12, 15, 35, 0).in_time_zone('London'), date_offer_withdrawn: DateTime.new(2019, 06, 12, 15, 35, 0).in_time_zone('London'),
                                          mobilisation_period: 4, optional_call_off_extensions_1: 1, optional_call_off_extensions_2: 1, optional_call_off_extensions_3: nil, optional_call_off_extensions_4: nil,
        ...
        @page_data[:procurement_data][:declined_reason] = 'conflict of interest or other reason, maybe a lot longer reason that usually is recorded here, but this is strictly for testing purposes and it would require a long declining reason.'
        @page_data[:procurement_data][:withdrawn_reason] = 'conflict of interest or other reason, maybe a lot longer reason that usually is recorded here, but this is strictly for testing purposes and it would require a long withdrawing reason.'
        ...
        @page_data[:procurement_data][:aasm_state] = find_aasm_state(request.path_info)
        @page_data[:procurement_data][:closed] = request.path_info.include?('closed')
      end
      ...
      def find_aasm_state(path)
        return :awaiting_supplier_response if path.include?('awaiting-response')
        return :awaiting_contract_signature if path.include?('awaiting-signature')
        return :accepted_and_signed if path.include?('accepted-signed')
        return :accepted_not_signed if path.include?('not-signed')
        return :supplier_declined if path.include?('declined')
        return :no_supplier_response if path.include?('no-response')
        return :supplier_declined if path.include?('closed-1')
        return :accepted_not_signed if path.include?('closed-2')
        return :no_supplier_response if path.include?('closed-3')
        return :withdrawn if path.include?('closed-4')
      end
      ...
      def set_continuation_text
        case find_aasm_state(request.path_info)
      ...
      def set_secondary_text
        if request.path_info.include?('closed') || find_aasm_state(request.path_info) == :accepted_and_signed
