    ...
    module Admin
      class SessionsController < Base::SessionsController
        ...
        def after_sign_in_path_for(resource)
          ...
          facilities_management_beta_admin_path
        end
        # rubocop:enable Lint/UnusedMethodArgument

        def after_sign_out_path_for(_resource)
          facilities_management_beta_admin_start_path
        end

        def new_session_path
          facilities_management_beta_admin_new_user_session_path
        end

        def confirm_forgot_password_path(username)
          facilities_management_beta_admin_edit_user_password_path(username: username)
        end

        def confirm_email_path(email)
          facilities_management_beta_admin_users_confirm_path(email: email)
        end
      end
    end
  end
