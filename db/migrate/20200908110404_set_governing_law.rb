class SetGoverningLaw < ActiveRecord::Migration[5.2]
  ...
  def up
    FacilitiesManagement::RM3830::Procurement.direct_award.where(governing_law: nil).each do |procurement|
      procurement.update(governing_law: DEFAULT_LAW)
    end

    FacilitiesManagement::RM3830::Procurement.da_draft.where(governing_law: nil).each do |procurement|
