...
INSERT INTO public.fm_units_of_measurement (id, title_text, example_text, unit_text, data_type, service_usage) VALUES(7, 'How many labour hours are required each year?', 'Example, 520. If this service is required for 10 hours per week, then enter 520 hours (each year)', '', 'numeric', '{H.4,H.5,I.1,I.2,I.3,I.4,J.1,J.2,J.3,J.4,J.5,J.6}');
INSERT INTO public.fm_units_of_measurement (id, title_text, example_text, unit_text, data_type, service_usage) VALUES(8, 'How many tonnes of waste need disposal each year?', ' ', 'tonnes (each year)', 'numeric', '{K.2,K.3,K.4,K.5,K.6}');
...
'
  ...
  def self.facilities_management_buildings
    ActiveRecord::Base.connection_pool.with_connection do |db|
      ...
      DROP INDEX IF EXISTS idx_buildings_user_id; CREATE INDEX idx_buildings_user_id ON facilities_management_buildings USING btree (user_id);"
      db.query query
    end
  rescue PG::Error => e
    puts e.message
