...
namespace :mc do
  ...
  task data: :environment do
    ...
    input = File.dirname(Rails.root.join('storage', 'management_consultancy', 'current_data', 'input'))
    FileUtils.mkdir_p(input) unless File.directory?(input)
    output = File.dirname(Rails.root.join('storage', 'management_consultancy', 'current_data', 'output'))
    FileUtils.mkdir_p(output) unless File.directory?(output)

    touch Rails.root.join('storage', 'management_consultancy', 'current_data', 'output', 'errors.out')
    add_suppliers
