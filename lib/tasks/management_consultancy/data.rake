...
namespace :mc do
  ...
  task data: :environment do
    ...
    # s3 = Aws::S3::Resource.new(region: ENV['COGNITO_AWS_REGION'], credentials: )
    ...
    #     run_script(s3, add_rate_cards_to_suppliers, './storage/management_consultancy/current_data/output/suppliers_with_service_offerings_and_regional_availability_and_rate_cards.json')
    add_suppliers
    add_service_offerings_per_supplier
    add_region_availability_per_lot_per_supplier
    add_rate_cards_to_suppliers
    cp './storage/management_consultancy/current_data/output/suppliers_with_service_offerings_and_regional_availability_and_rate_cards.json', './storage/management_consultancy/current_data/output/data.json'
  end
end
