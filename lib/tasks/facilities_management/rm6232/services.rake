module FM::RM6232
  module Services
    def self.truncate_tables
      ...
      FacilitiesManagement::RM6232::Sector.destroy_all
    end
    ...
    def self.add_sectors
      file_name = 'data/facilities_management/rm6232/sectors.csv'

      CSV.read(file_name, headers: true).each do |row|
        FacilitiesManagement::RM6232::Sector.create(row.to_h)
      end
    end
  end
end
...
namespace :db do
  namespace :rm6232 do
    ...
    task import_services: :environment do
      DistributedLocks.distributed_lock(191) do
        ActiveRecord::Base.transaction do
          ...
          FM::RM6232::Services.add_sectors
    ...
    task static: :import_services do
    end
  end

  desc 'add Services for RM6232 to the database'
  task static: :'rm6232:import_services' do
  end
end
