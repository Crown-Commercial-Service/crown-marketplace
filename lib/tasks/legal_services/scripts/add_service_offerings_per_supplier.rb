...
def add_service_offerings_per_supplier
  ...
  service_offerings_workbook = Roo::Spreadsheet.open(Rails.root.join('storage', 'legal_services', 'current_data', 'input', 'Service offerings.xlsx'), extension: :xlsx)

  (0..12).each do |sheet_number|
    sheet = service_offerings_workbook.sheet(sheet_number)
    service_names = sheet.column(1)
    lot_number = extract_service_number(service_names[1]).match(/LS.\d[.]\d+/).to_s
    ...
    (2..sheet.last_column).each do |column_number|
      ...
      supplier = suppliers.find { |s| s['DUNS Number'] == supplier_duns.to_s }
      supplier['lots'] << { 'lot_number' => lot_number, 'services' => service_offerings } if supplier
    end
  end

  File.open('/Users/mike/feature/LS-143/storage/legal_services/current_data/output/suppliers_with_service_offerings.json', 'w') do |f|
