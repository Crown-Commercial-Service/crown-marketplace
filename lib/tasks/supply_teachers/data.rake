...
namespace :st do
  task :clean do
    ...
    SupplyTeachers::Admin::CurrentData.first.update(data: nil, error: nil)
  end
  ...
  def upload_data_and_errors
    ...
    current_data.data = File.open(get_output_file_path('data.json'))
    current_data.error = get_output_file_path('errors.out').read
    current_data.save!
  end
