...
def exclude_suppliers_without_accreditation
  suppliers = JSON.parse(File.read('./storage/supply_teachers/output/data_merged.json'))
  suppliers_with_accreditation = suppliers.select { |s| s['accreditation'] }

  File.open('./storage/supply_teachers/output/data_only_accredited.json.tmp', 'w') do |f|
